...
impl<FatalError: Error> std::fmt::Display for WriteError<FatalError> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            WriteError::BufferTooLong { buffer_len, block_size } => {
                write!(f, "Buffer with len {} is longer than the block size {}", buffer_len, block_size)
            }
            WriteError::InvalidBuffer => {
                write!(f, "Buffer does not match internal buffer layout")
            }
            ...
            WriteError::UnknownFatalError => write!(f, "An unkown fatal error occurred"),
        }
    }
}
